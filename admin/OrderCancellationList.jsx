...
const OrderCancellationList = () => {
  ...
  const generateReport = () => {
    ...
    // Remove the 'Delete' column (index 4) from each row
    const rowsWithoutDeleteColumn = rows.map((row) => row.slice(0, 4));

    // Set the table headers
    const headers = ['Order ID', 'Product ID', 'Cancellation Date', 'Title for Cancellation', 'Reason for Cancellation'];

    // Add the table to the PDF document
    doc.autoTable({ head: [headers], body: rowsWithoutDeleteColumn });
